var save = {
	...
	save : function(){
		...
		htmlInteraction.disableButton('save-button');
		...
		var vars = { 'data': JSON.stringify(save_data)};
		var existing = false;
		...
		$.ajax({
			...
			data: vars,
				...
				htmlInteraction.enableButton('save-button');
				var json = JSON.parse(response);
				if(!json['success']){
					console.log(json['error']);
					alert('Something went wrong saving stuff :-(');
				}else{
					main.key = json['key'];
					htmlInteraction.setInnerHtml('save-message','Load your save <a id="save-link" href="http://tommedley.com/spacerace/?key='+json['key']+'">here</a> (bookmark this link).')
					try{
						window.history.replaceState("", "Warp", "?key="+json['key']);
					}catch(e){
						console.log(e.message);
					}
					if(!existing){
						alert('Save successful! You can get back to your game with the key: ' + json['key']);
					}else{
	...
	load : function(key){
		...
		$.ajax({
			...
			data: { 'key': key },
				...
				if(!json['success']){
					console.log(json['error']);
					alert('Something went wrong loading stuff :-(');
				}else{
